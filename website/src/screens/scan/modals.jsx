...
export function RetakeReasonModal({ open, image, backendReason, onConfirm, onCancel }) {
  ...
  return (
    <Modal open={open} onOpenChange={(next) => !next && onCancel()} title="Is this photo usable?">
      {image && (
        <img src={image.dataUrl} alt="Captured label" className="mx-auto mb-3 max-w-[280px] rounded-xl border border-border" />
      )}
  ...
  return (
    <Modal open={open} onOpenChange={(next) => !next && onClose()} title={`Correct "${field.name}"`}>
      ...
      {!evidence ? (
        ...
        <>
          <img src={evidence.dataUrl} alt="Evidence for correction" className="w-full rounded-xl border border-border" />
  ...
  return (
    <Modal open={open} onOpenChange={(next) => !next && onClose()} title="Confirm violation" wide>
      ...
      {!showOverride ? (
        ...
        <div className="mt-4 rounded-xl border border-border bg-panel-alt p-4">
...
export function HoldNoticeModal({ open, scanMeta, onIssue, onClose }) {
  ...
  return (
    <Modal open={open} onOpenChange={(next) => !next && onClose()} title="Signed hold notice">
      {!issued ? (
        <>
          <div className="rounded-xl border border-border bg-panel-alt p-4 font-mono text-[12.5px] leading-relaxed text-ink">
          ...
          <canvas
            ...
            width={280}
            height={100}
            className="w-full cursor-crosshair rounded-xl border border-dashed border-border bg-white [touch-action:none]"
...
// against pre-loaded demo products and that product's model is shown.
        ...
        <>
          ...
          {modelViewerReady ? (
            <button
              ...
              className="group relative block w-full cursor-zoom-in overflow-hidden rounded-xl border-0 p-0"
            ...
            >
              ...
              />
              <span className="absolute right-2 top-2 flex items-center gap-1 rounded-xl bg-navy-deep/70 px-2 py-1 text-[11px] font-medium text-white opacity-90 transition-opacity group-hover:opacity-100">
                <Maximize2 size={12} /> Enlarge
              </span>
            </button>
          ) : (
            <div className="flex h-[320px] items-center justify-center gap-2 rounded-xl bg-panel-alt text-[13px] text-ink-soft">
