...
import { formatDate, todayLocalISO } from "../lib/format.js";
...
function ClauseRow({ clause, editable, onSave, onRemove }) {
  ...
  if (editing) {
    return (
      <div className="flex flex-col gap-2 rounded-xl border border-navy bg-panel-alt p-3">
        <textarea
          className="w-full rounded-xl border border-border px-2.5 py-2 font-mono text-[12.5px] focus:border-navy focus:outline-none"
          rows={2}
          value={draft}
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-border bg-panel px-3 py-2.5">
      ...
      {editable && (
        <div className="flex shrink-0 gap-1">
          <button type="button" aria-label="Edit clause" className="rounded-xl p-1.5 text-ink-soft hover:bg-panel-alt hover:text-navy" onClick={() => setEditing(true)}>
            <Pencil size={14} />
          </button>
          <button type="button" aria-label="Remove clause" className="rounded-xl p-1.5 text-ink-soft hover:bg-red-soft hover:text-red" onClick={onRemove}>
            <Trash2 size={14} />
          </button>
        </div>
      )}
  ...
  return (
    <div className="flex flex-col gap-1 font-mono text-[11.5px] leading-relaxed">
      {diff.removed.map((line, i) => (
        <div key={"r" + i} className="rounded-xl bg-red-soft px-2 py-1 text-red">− {line}</div>
      ))}
      {diff.added.map((line, i) => (
        <div key={"a" + i} className="rounded-xl bg-green-soft px-2 py-1 text-green">+ {line}</div>
      ))}
    </div>
  );
}
  ...
  return (
    <div className="mb-1 flex flex-col gap-3 rounded-lg border border-brass/50 bg-gradient-to-br from-panel to-panel-alt p-4 shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="rounded-full bg-brass-soft px-2 py-0.5 text-[12px] font-semibold uppercase tracking-wide text-brass">AI extraction</span>
      </div>
      ...
      <button
        ...
        disabled={uploading}
        className="flex items-center justify-center gap-2 rounded-[22px] border-2 border-dashed border-brass/50 bg-panel/70 py-3 text-[12.5px] font-medium text-ink transition-colors hover:border-brass hover:bg-brass-soft disabled:cursor-not-allowed disabled:opacity-60"
      >
        {uploading ? (
        ...
        <div className="flex flex-col gap-1.5">
          {sources.map((s, i) => (
            <div key={i} className="flex flex-col gap-1.5">
              <div className="flex items-start gap-2 rounded-xl border border-green/20 bg-green-soft px-3 py-2 text-[12px] text-green">
              ...
              {/* Bug fix: clauses Gemini found but couldn't turn into a usable
                  line (e.g. an empty body) used to just vanish from the count
                  with no sign anything was lost — this surfaces them
                  separately from the 25-clause cap above. */}
              {!!s.malformedCount && (
                <InlineBanner tone="error">
                  {s.malformedCount} requirement{s.malformedCount === 1 ? "" : "s"} in <strong>{s.filename}</strong> could
                  not be normalized into a usable clause and were not added to this draft. Review the document yourself
                  for anything missed.
                </InlineBanner>
              )}
            </div>
...
export function RuleAdminView({ ruleVersions, onPublish, adminName = "Rule Admin", scans = [] }) {
      ...
      setRegulaSync((s) => ({
        ...
        sources: [
          ...s.sources,
          {
            ...
            malformedCount: result.malformedCount || 0,
          },
        ],
      }));
    } catch (err) {
  ...
  async function runImpactPreview() {
    ...
    try {
      // Bug fix: this fetch's response used to go unchecked — a failed
      // registration (network blip, server restart at just the wrong
      // moment) meant every analyzeLabelImage call below silently fell back
      // to the base rules, and the preview would report a false "0 of N
      // cases would change" all-clear for a draft that was never actually
      // tested.
      const registerRes = await fetch("/api/rules", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ versions: [{ version: previewVersion, ruleText: previewRuleText }] }),
      });
      if (!registerRes.ok) {
        throw new Error("Could not register this draft with the server (HTTP " + registerRes.status + ") — the preview was not run, so no result would have reflected this draft.");
      }
      const sample = scans.filter((s) => s.imageDataUrl && s.status !== "retake_needed" && Array.isArray(s.fields)).slice(0, 5);
      const perCase = [];
      let ruleVersionMissing = false;
      for (const s of sample) {
        ...
        try {
          ...
          if (parsed.ruleVersionApplied === false) ruleVersionMissing = true;
          const flips = [];
      ...
      setImpact({
        running: false,
        result: { sampleSize: sample.length, changedCount, perCase, forRuleText: previewRuleText, forActiveRuleText: activeVersion.ruleText, ruleVersionMissing },
        error: null,
      });
    } catch (e) {
    ...
    <div className="lm-panel">
      ...
      {!drafting ? (
        ...
        <div className="lm-ruleadmin-draft">
          ...
          <div className="mb-3 mt-2 flex gap-1 rounded-[22px] border border-border bg-panel-alt p-1">
            <button
              type="button"
              className={"flex-1 rounded-xl px-3 py-1.5 text-[12.5px] font-semibold transition-colors " + (editorMode === "catalog" ? "bg-panel text-ink shadow-sm" : "text-ink-soft")}
            ...
            <button
              type="button"
              className={"flex-1 rounded-xl px-3 py-1.5 text-[12.5px] font-semibold transition-colors " + (editorMode === "raw" ? "bg-panel text-ink shadow-sm" : "text-ink-soft")}
                ...
                <>
                  ...
                  <div className="flex max-h-[340px] flex-col gap-3 overflow-y-auto pr-1">
                      ...
                      <div key={group.category} className="flex flex-col gap-2">
                        ...
                        {group.clauses.map((clause) => (
                          <ClauseRow
                            ...
                            clause={clause}
                            editable
                            onSave={(newRaw) => { setRuleText((current) => updateClauseText(current, clause.index, newRaw)); setPending(null); }}
                            onRemove={() => { setRuleText((current) => removeClauseAt(current, clause.index)); setPending(null); }}
                          />
                        ))}
                      </div>
                    ))}
                    {addingClause ? (
                      <div className="flex flex-col gap-2 rounded-xl border border-navy bg-panel-alt p-3">
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() => { setRuleText((current) => addClauseLine(current, newClauseText)); setNewClauseText(""); setAddingClause(false); setPending(null); }}
                          >
                            <Check size={13} /> Add
                          </Button>
                      ...
                      <button
                        type="button"
                        className="flex items-center justify-center gap-1.5 rounded-xl border border-dashed border-border py-2.5 text-[12.5px] font-medium text-ink-soft hover:border-navy hover:text-navy"
          ...
          <div className="mt-4 rounded-[22px] border border-border bg-panel p-3.5">
            ...
            {impact.result?.ruleVersionMissing && (
              <InlineBanner tone="error">
                The server couldn't find this draft's rule version during at least one re-check (it may have been lost
                to a backend restart) — one or more cases below were compared against the base rules only, not this
                draft. Re-run the preview before trusting this result.
              </InlineBanner>
            )}
            {impact.result && (
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <div className="flex min-w-[200px] flex-1 items-center gap-2 rounded-xl border border-border bg-panel px-2.5 py-1.5">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 rounded-[22px] border border-border bg-panel-alt p-3">
        <Columns2 size={14} className="text-ink-soft" />
        <span className="text-[12px] font-semibold text-ink-soft">Compare</span>
        <select className="rounded-xl border border-border bg-panel px-2 py-1 text-[12.5px]" value={compareA || ""} onChange={(e) => setCompareA(e.target.value || null)}>
        ...
        <select className="rounded-xl border border-border bg-panel px-2 py-1 text-[12.5px]" value={compareB || ""} onChange={(e) => setCompareB(e.target.value || null)}>
      ...
      {compareDiff && (
        <div className="mb-4 rounded-[22px] border border-border bg-panel p-3.5">
            ...
            <div key={r.version} className="lm-changelog-item">
              <div className="lm-changelog-top">
                ...
                <div className="ml-auto flex items-center gap-1">
                  <button type="button" aria-label="Download version as PDF" className="rounded-xl p-1.5 text-ink-soft hover:bg-panel-alt hover:text-navy" onClick={() => downloadRuleVersionPdf(r)}>
                    <FileDown size={14} />
                  </button>
                  {!isActive && (
                    rollbackConfirm?.version === r.version ? (
                      ...
                      <button type="button" aria-label="Roll back to this version" className="rounded-xl p-1.5 text-ink-soft hover:bg-brass-soft hover:text-ink" onClick={() => askRollback(r)}>
                        <RotateCcw size={14} />
                      </button>
                    )
                  )}
