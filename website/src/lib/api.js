...
export async function verifyCorrection({ evidenceDataUrl, fieldName, correctedValue }) {
  try {
    ...
    const data = await res.json();
    // Bug fix: this used to skip the response.ok check every other function
    // in this file does, so a 4xx/5xx error body (e.g. {error: "..."}) was
    // parsed as if it were a real result — {plausible: undefined, note:
    // undefined} — instead of this function's own documented "unavailable"
    // failure shape, which callers checking `=== null` won't recognize.
    if (!res.ok) {
      return { plausible: null, note: (data && data.error) || "Verification unavailable." };
    }
